...
class LinkedTitelListe implements TitelListe
{
    ...
    public void fuegeEin(Titel titel, int position)
    {
        ...
        DoppellinkKnoten hilfsKnoten = new DoppellinkKnoten();
        if(position < _anzahlTitel / 2){
            hilfsKnoten = _listenkopf;
            for(int i = 0; i < position; i++){
                hilfsKnoten = hilfsKnoten.gibNachfolger();
            }
            DoppellinkKnoten knoten = new DoppellinkKnoten(titel, hilfsKnoten, 
            hilfsKnoten.gibNachfolger());
            hilfsKnoten.setzeNachfolger(knoten);
            knoten.gibNachfolger().setzeVorgaenger(knoten);
            _anzahlTitel += 1;
        }else{
            hilfsKnoten = _listenende;
            for(int i = _anzahlTitel; i > position; i--){
                hilfsKnoten = hilfsKnoten.gibVorgaenger();
            }
            DoppellinkKnoten knoten = new DoppellinkKnoten(titel, 
            hilfsKnoten.gibVorgaenger(), hilfsKnoten);
            hilfsKnoten.setzeVorgaenger(knoten);
            knoten.gibVorgaenger().setzeNachfolger(knoten);
            _anzahlTitel += 1;
        }
    }
    ...
    public boolean enthaelt(Titel titel)
    {
        ...
        for(int i = 0; i < _anzahlTitel; i++){
            hilfsKnoten = hilfsKnoten.gibNachfolger();
            if(hilfsKnoten.gibTitel().equals(titel)){
                return true;
            }
        }
        return false;
    ...
    public void entferne(int position)
    {
        ...
        hilfsKnoten.setzeNachfolger(hilfsKnoten.gibNachfolger().gibNachfolger());
        _anzahlTitel -= 1;
    }
